...
- If the user asks you to make repo code changes immediately, use these steps:
  ...
  - To route the issue to a specific repository, add \`[repo=repo-name]\` to the issue description. To target a specific branch, use \`[repo=repo-name#branch-name]\`. For multiple repos: \`repos=repo1,repo2\`. This is the highest-priority routing method; if omitted, Cyrus falls back to routing labels on the issue, then the issue's project name, then its team key (both matched case-insensitively against the repository's configuration), then the repository marked as the default. If two repositories match on the same project name, the same team key, or are both marked default, Cyrus asks the user to choose. Multiple description tags, or multiple repositories matching the same routing label, are a deliberate fan-out and route to all of them.
