...
### Added
- Router logs can now be exported over OpenTelemetry, so they land in a log backend as structured records with queryable fields instead of prose to be pattern-matched. Set `CYRUS_OTEL_LOGS_ENABLED=true` and point `APPLICATIONINSIGHTS_CONNECTION_STRING` at your endpoint; `CYRUS_OTEL_LOGS_LEVEL` controls how much is sent (default `INFO`), independently of what the router prints locally. Every record carries the service name, version, instance, deployment environment and cloud region, so you can tell one replica or one environment from another. Off unless you turn it on, and the existing console output is unchanged either way — this is an additional destination, not a replacement. On Azure the Terraform stack wires it up for you, into the same Log Analytics workspace the router's stdout already goes to. ([NOR-281](https://linear.app/northrop-digital/issue/NOR-281/telemetry-phase-3-vendor-neutral-opentelemetry-logging-foundation), [#28](https://github.com/nick-boey/cyrus/pull/28))
- Logs from cloud sandbox workers now reach the router's log stream instead of disappearing with the sandbox. Previously nothing collected a worker's output, so when a session went wrong in a cloud container the only record was whatever it managed to post to Linear. Warnings and errors are forwarded by default, tagged with the issue and device they came from; `CYRUS_LOG_FORWARD_LEVEL`, `CYRUS_LOG_FORWARD_RATE` and `CYRUS_LOG_FORWARD_BURST` adjust how much is sent, and anything the rate guard skips is reported as a count rather than silently dropped. Nothing is sent until the router confirms it can accept it, so a worker running against an older router simply keeps its logs to itself. ([NOR-280](https://linear.app/northrop-digital/issue/NOR-280/telemetry-phase-2-ship-sandbox-worker-logs-off-the-box), [#21](https://github.com/nick-boey/cyrus/pull/21))
