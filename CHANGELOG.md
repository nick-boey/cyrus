...
### Added
- Claude sessions can now keep your Anthropic credentials out of the environment of every shell command the agent runs. Set `CYRUS_SUBPROCESS_ENV_SCRUB=1` and each session's tool invocations run under a sandbox that strips those variables before handing the environment to a subprocess. It is off unless you ask for it, and when you do ask for it on a machine that cannot provide it, the session fails immediately naming what is missing and how to install it — rather than starting anyway without the protection, which is what the previous behaviour amounted to. Cloud sandboxes now ship the two packages it needs (`socat` and `bubblewrap`); on your own machine, install them with your package manager. ([NOR-412](https://linear.app/northrop-digital/issue/NOR-412/subprocess-env-scrubbing-is-off-in-every-sandbox-socat-and-bubblewrap))
