...
### Fixed
- Closing an issue now actually cleans up after it. Cyrus keeps a snapshot of a session's uncommitted work on your remote so a session that loses its machine can pick up where it left off, and deletes that snapshot when the issue is marked Done or Canceled — but it was looking for the session's files in the wrong place, so it deleted nothing. Every snapshot Cyrus had ever taken was still on the remote: these are sent to everyone who clones or fetches the repository and count against its size limit, so the cost grew with every issue and fell on every contributor. The same mistake meant a workspace on your own machine was left on disk if you had pointed Cyrus's worktrees somewhere other than its default directory. Both are fixed, and when Cyrus does find nothing to clean up it now says whether that is because the work was already gone or because it could not tell where to look — previously the two were indistinguishable, which is why this went unnoticed. Snapshots left behind by earlier versions are not removed automatically; list them with `git ls-remote origin 'refs/cyrus-wip/*'` and delete the ones for closed issues with `git push origin --delete <ref>`. ([NOR-411](https://linear.app/northrop-digital/issue/NOR-411/wip-snapshot-refs-leak-permanently-teardown-looks-for-the-worktree))
