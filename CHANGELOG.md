...
- An issue whose agent session stopped working is no longer silently unreachable. When a session never reaches a terminal state — most often because the agent left a background task running, which withholds the completion signal indefinitely — it keeps holding the issue, and a new top-level `@cyrus1` comment on that issue is rejected rather than delivered. That rejection now says so plainly on the issue: if the running session is your own, it tells you the comment did not reach it and that replying **inside that session's thread** will, instead of the previous message claiming the session belonged to another user, which was usually the same person. If the session was started by someone else, it says that instead — the old advice sent you round in a circle, because replying in someone else's thread is refused too, and being told to start your own session brought you straight back to the locked issue. Operators also get a warning in the logs naming the session that holds the issue, so a dropped comment is distinguishable from a webhook that never arrived, and that warning now covers replies inside an existing thread as well as new top-level comments. ([NOR-402](https://linear.app/northrop-digital/issue/NOR-402/a-never-terminating-agent-session-makes-an-issue-permanently), [#49](https://github.com/nick-boey/cyrus/pull/49))
>>>>>>> 7cbc439e (Fix all findings from the implementation review (NOR-402))
...
- `cyrus router unlock` now frees the sandbox as well as the issue. It used to release only the lock, leaving the finished session's claim on its container in place — so the container was never stopped or cleaned up, and the stranded-session alert kept firing every minute with nothing left to try. Because it now ends the session's claim outright, confirm the session is genuinely stuck rather than waiting on a scheduled wakeup before running it; the alert and the log line both say how. ([NOR-402](https://linear.app/northrop-digital/issue/NOR-402/a-never-terminating-agent-session-makes-an-issue-permanently), [#49](https://github.com/nick-boey/cyrus/pull/49))
