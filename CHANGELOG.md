...
- The device token `cyrus connect` already gave you now also authenticates against the operator context route, at exactly the authority it already had: read-only, and scoped to your own work. Tokens held inside cloud sandboxes are refused outright — a credential issued for one issue gains no fleet-wide reach. ([CYR-65](https://linear.app/northrop-digital/issue/CYR-65/authenticate-scoped-fleet-operators-and-expose-discovery), [#58](https://github.com/nick-boey/cyrus/pull/58))
