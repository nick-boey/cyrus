...
- Router mode: a session's final summary now reaches Linear. The last message of every completed session was being rejected and lost, so sessions appeared to stop mid-thought.
- Router mode: sessions that schedule follow-up work (a wakeup, a background task) no longer go dark when the first turn ends. Everything the session did after that point — sometimes many minutes of work — was silently discarded and never appeared on the issue. The router does this itself when it accepts a delegation, before handing the work to your device — an issue that is already started, one whose creator has no enrolled device, and one already locked by another session are all left alone. A previous release claimed to have fixed this by rebuilding the issue's team and state on the device, but the ids those depend on never survived the trip from the router, so the promotion silently did nothing.
