...
### Changed
- The Codex model list on `/setup` now offers **GPT-5.6 Sol**, and prefers it for new selections. GPT-5.5 is still on the list, so if you had already chosen it nothing changes and there is nothing to do. The list stays deliberately short: OpenAI serves a narrower set of models to a ChatGPT subscription than to a paid API key, and a name it refuses is a session that simply dies, so only names verified against a real subscription are offered. Sessions that do not name a model themselves now get GPT-5.6 Sol; previously `/setup`, the session, and the model named in the timeline could each say something different. ([CYR-79](https://linear.app/northrop-digital/issue/CYR-79/make-codex-subscription-auth-independent-of-the-default-runner-and), [#55](https://github.com/nick-boey/cyrus/pull/55))
