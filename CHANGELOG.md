...
### Fixed
- Rotating a stored credential — a Claude Code OAuth token, a GitHub token — now actually takes effect on an issue that already has a cloud container. Previously the new value never arrived: starting a new session reused the existing container, and even stopping and deleting it by hand didn't help, because the snapshot taken when the container parked put the old credential straight back. The container is now rebuilt automatically when your stored credentials have changed, and the issue gets a note saying so. Uncommitted work is restored from the issue's WIP snapshot, so nothing is lost — the session just takes longer than usual to start. Note that a credential set from the command line can take up to a minute to be picked up, and that a container is rebuilt even if it is mid-session, so rotate between sessions rather than while the agent is working.
- `cyrus router containers destroy <issueKey>` now really destroys the cloud sandbox and its snapshots, instead of only forgetting about them and promising a later cleanup that adopted the abandoned sandbox on the issue's next event. If the teardown fails, the issue keeps its record so you can retry. For local Docker containers it now says plainly that it has left the container to the background sweep rather than implying it removed it.
