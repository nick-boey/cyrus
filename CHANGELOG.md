...
- Cloud workers no longer keep running — and billing — after their issue is finished. A worker that had accumulated a large amount of state took longer to park than the router was willing to wait, so parking failed, was retried every minute, and each retry queued up behind the last. Cleanup for a closed issue never got a turn, and the sweep that parks *every other* idle worker stalled behind it too. Workers now park reliably regardless of how much state they hold, and one slow worker can no longer hold up the rest.
