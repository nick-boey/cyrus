...
### Fixed
- The "Set up your account" button on `/setup` now works. On Azure deployments it failed with a bare 403 and no explanation: the sign-in sidecar rejected the click before it ever reached Cyrus, because a plain form submission carries none of the headers the sidecar requires to tell a real click apart from a forged one. Nobody could create their own account through the browser at all. ([#18](https://github.com/nick-boey/cyrus/pull/18))
