...
### Added
- Logs can now be emitted as one JSON object per line by setting `CYRUS_LOG_FORMAT=json`, so a log aggregator can filter on level, component, and issue rather than pattern-matching prose. The default output is unchanged.
- The router now logs when a device connects and disconnects, and why a handshake was rejected. For cloud workers this is the first sign the worker process actually started — the infrastructure can report a sandbox as "running" while nothing is listening inside it.
...
### Fixed
- Router failures that previously appeared as warnings — a failed container boot, a lost delegation, a state-backup failure, a sweep that could not run — are now reported as errors and carry the underlying stack trace instead of a one-line flattened message.
