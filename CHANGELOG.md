...
- A client resuming the change feed with a position from before a router restart now gets an explicit "this stream is gone" rather than an empty, successful-looking answer it would read as "nothing happened". It can re-list and resume from a fresh position. A position presented against a different query, or one that was not issued by the router, is refused the same way — as a bad request, distinct from a lost stream, so an automated caller can tell "fix the request" from "start again". ([CYR-69](https://linear.app/northrop-digital/issue/CYR-69/serve-paginated-run-snapshots-and-durable-material-changes), [#62](https://github.com/nick-boey/cyrus/pull/62))
