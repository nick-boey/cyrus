...
- The saved query the stranded-sandbox alert directs operators to now works. It contained a syntax error that would have surfaced only when someone opened it mid-incident, and, once corrected, would still have missed the most common case: a session that finished normally at least once before it got stuck. Since replying in an existing session's thread is the recommended way to work around a locked issue, the workaround itself produced the shape the query could not see — and an empty result was the operator's signal to unlock the issue, which is the one action that makes a waiting session worse. ([CYR-1](https://linear.app/northrop-digital/issue/CYR-1/a-never-terminating-agent-session-makes-an-issue-permanently))
