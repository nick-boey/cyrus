...
- Container sessions can now lint GitHub Actions workflows with `actionlint`, drive a browser with Playwright (Chromium is preinstalled, so it works even under deny-by-default network egress), and use the Fleece CLI. These are available in both container executors — the same worker image backs local Docker and Azure Container Apps.
