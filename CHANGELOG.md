...
### Added
- A session that is waiting on you now says so, and says so separately from whether its container can be suspended. Previously the two were one signal: a session that asked you a question while a background build was still running reported nothing at all, so from the outside it was indistinguishable from a worker that had stopped. It now reports that it is waiting either way, and simply declines to let its container be suspended while the build runs — because suspending it would freeze that build, and its completion is what would have woken the session. ([CYR-68](https://linear.app/northrop-digital/issue/CYR-68/capture-routing-and-worker-reported-run-facts), [#61](https://github.com/nick-boey/cyrus/pull/61))
- A session held open for hours by a scheduled wakeup, a cron, or a background task now reports itself as active work with a count of what is holding it open, instead of going quiet after its turn ends. A seven-hour run is no longer indistinguishable from a stalled one. ([CYR-68](https://linear.app/northrop-digital/issue/CYR-68/capture-routing-and-worker-reported-run-facts), [#61](https://github.com/nick-boey/cyrus/pull/61))
- Runs now record the workspace, owner, Linear team, and Linear project they were routed under, along with which agent and model ran them. These are captured at the moment work is routed and never rewritten, so moving an issue to another team or project no longer changes the history of runs that already happened. ([CYR-68](https://linear.app/northrop-digital/issue/CYR-68/capture-routing-and-worker-reported-run-facts), [#61](https://github.com/nick-boey/cyrus/pull/61))
