...
### Fixed
- If you run the Azure router, a cloud sandbox left holding an issue it stopped working on is now cleaned up on its own. A session that never finishes properly keeps its claim on the sandbox forever, and that claim was enough to make the sandbox invisible to every cleanup there was — so it kept running, and kept costing, until somebody noticed by hand. A sandbox that nothing has touched for a day — nothing sent to it, no sign of life from it, nothing posted by its agent, no new session on it — is now shut down and removed along with its snapshots, and the issue it was holding is released so you can work on it again. Anything still showing signs of life is left alone, and a session that picks the sandbox back up while the cleanup is being decided cancels it. Set `containers.reclaimStrandedMs` to change the day, or to `0` to turn it off. ([CYR-84](https://linear.app/northrop-digital/issue/CYR-84/sweep-stale-aca-sandboxes-and-obsolete-disk-images-automatically), [#75](https://github.com/nick-boey/cyrus/pull/75))
- If you run the Azure router, old worker images no longer pile up. Every deployment registers a new image, and nothing ever removed the old ones — one audited deployment had built up 14 images taking 68 GB, more than half of which nothing was using. Images with nothing running on them, nothing restoring from them, and no issue pinned to them are now removed once they are a week old. The week is deliberate: the image you are about to deploy and the one you would roll back to are both, at that moment, used by nothing, so age is what tells them apart from junk. The current deployment image is never removed, whatever its age. Set `containers.aca.imageRetentionMs` to change the week and `containers.aca.imageGcIntervalMs` to change how often the check runs. ([CYR-84](https://linear.app/northrop-digital/issue/CYR-84/sweep-stale-aca-sandboxes-and-obsolete-disk-images-automatically), [#75](https://github.com/nick-boey/cyrus/pull/75))
