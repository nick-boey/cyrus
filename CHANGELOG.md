...
### Added
- Claude sessions can now be asked to keep your Anthropic credentials out of the environment of every shell command the agent runs. Set `CYRUS_SUBPROCESS_ENV_SCRUB=1` on the machine running Cyrus and each session's tool invocations are run under a sandbox that strips those variables before handing the environment to a subprocess. It is off unless you ask for it, and when you do ask for it on a machine that cannot provide it, the session fails to start and says what is missing and how to install it — rather than starting anyway without the protection, which is what the previous behaviour amounted to. It needs `socat` and `bubblewrap`; install them with your package manager. Cloud sandboxes will carry both from the next worker image build, so leave the setting off there until that image is deployed. ([NOR-412](https://linear.app/northrop-digital/issue/NOR-412/subprocess-env-scrubbing-is-off-in-every-sandbox-socat-and-bubblewrap))
