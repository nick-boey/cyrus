...
- Recovery is off unless a router is explicitly configured for it, and a router that turns it on without something to run it refuses to start rather than accepting requests nothing acts on. While it is off, the capability is not advertised and every request is refused — which is the state every router ships in today, until the reconciliation it drives is built and verified. ([CYR-74](https://linear.app/northrop-digital/issue/CYR-74/persist-authorize-and-audit-recovery-operations), [#68](https://github.com/nick-boey/cyrus/pull/68))
