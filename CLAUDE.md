...
12. **ACA executor and Azure router invariants**:
   ...
   - `containers.keyVaultUrl` selects the Key Vault secret backend. Entra enrollment uses one app registration/audience per router deployment.
   - **A rotated per-user secret reaches an existing sandbox by REPLACING it, never by updating it** — ACA has no verb that changes a live sandbox's environment. `ContainerTargets.buildEnv` fingerprints the user's stored bundle, and `AcaSandboxesProvider` stamps it as the `cyrus.secrets` label and treats a mismatch exactly like a stale `cyrus.disk` label: the sandbox and its snapshots are deleted and a fresh one is created from the image, even if it is `Running`. This is load-bearing on three paths that otherwise resurrect the old value — a `Running` sandbox is returned as-is, a parked one resumes from a memory image holding the old env, and an absent one restores from a snapshot whose env is inherited by design. Both secret-store backends cache bundles for 60s in-process, so a `secrets set` from the CLI (a different process) can take up to a minute to be seen. Only the user's stored bundle is fingerprinted, never the router-controlled `CYRUS_*` vars — folding `CYRUS_REPOS_JSON` in would make a repository-registry edit start deleting live sandboxes. A `bootForTeardown()` wake deliberately passes no fingerprint. See `docs/adr/0002-credential-rotation-replaces-the-container.md`.
   - `cyrus router containers destroy <issueKey>` destroys the provider's resources first and deletes the device row only if that succeeded (the row is the retry handle). It reaches only the ACA provider; for docker/fly it says so and leaves the container to the orphan sweep.
