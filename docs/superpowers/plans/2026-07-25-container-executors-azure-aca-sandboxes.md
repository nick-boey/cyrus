...
- **Suspend/resume/snapshot:** `POST /stop` suspends (memory mode default: full memory+disk state → sub-second resume claim via `POST /resume`; measured in S3). `POST /snapshot` captures an explicit group-scoped snapshot; a new sandbox can be created from a snapshot (resource tier + entrypoint + env **reportedly** inherited — env/token inheritance is load-bearing and explicitly verified in spike S3b, B5). Snapshots are **not garbage-collected by Azure** — retention is our problem. Reportedly free during preview, billed as blob storage afterward — capture the pricing-page citation during Task 0 (n4).
...
- **Scale/cost:** per-second vCPU+memory billing (Consumption-plan rates), scale-to-zero while suspended; thousands of concurrent sandboxes; group `maxSandboxCount` cap. Defaults: 1 vCPU / 2 GiB / 300s auto-suspend (which we disable — N5). Tiers up to 4 vCPU / 8 GiB / 80 GB.
...
- Naming: sandbox `cyrus-issue-<key sanitized per S5>`; labels `cyrus.issue=<exact key>` (source of truth for `listManaged`), `cyrus.managed=true`, `cyrus.disk=<disk name>` (disk name encodes the image tag → staleness check), **`cyrus.device-id=<ctx.deviceId>` on sandboxes AND explicit snapshots (B5/D3)**.
- `ensureRunning(ctx)`:
...
- **Snapshot cost post-preview** (blob rates, citation captured in Task 0 — n4) — `keepSnapshots` pruning + `gc-snapshots` + group `maxSandboxCount` bound it.
